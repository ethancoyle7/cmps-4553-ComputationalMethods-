...
for f in DataFile:
    ...
    with open(f, 'r') as file:
        for line in file:
            # to read throughout the file create split to read
            # through the white space and on to the next word
            for data in line.split():
                FileStrings.add(data)

# Now to check and see if the FileStrings are present in other files
# according to our program,
for data in FileStrings:
    FileIndexer[data] = []  # where the word is occurring
    for f in DataFile:
...
# in tune with the example if the occurred words occur in multiple filter
