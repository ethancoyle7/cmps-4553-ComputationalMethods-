...
AlcoholConsumption2018 = AlcoholConsumption2018.reset_index().set_index('Country')
AlcoholConsumption2018.drop(columns={'index', "Wine", "Beer", "Spirits"}, inplace=True, axis=1)

#  drop and discard the index and the other columns we are not going to use from alcohol consumption file
AlcoholConsumption2018.rename(columns={'total_litres_of_pure_alcohol': 'Alcohol Consumption'}, inplace=True)
# we rename the collum of the total litres of pure alcohol to a easier to read format and set the
# inplace to be true

# set the indexing to be all lower case because they mismatching names on some of them
...
# as per guidelines, we dont use ascending order and make this inplace
...
mf['Alcohol Consumption'] = pd.cut(mf['Alcohol Consumption'], 2, labels=("Below Avg", "Above Avg"))
...
EquivalencyCountries = (mf['2018 GDP'] == mf['Alcohol Consumption']).sum()
WhereEquivalent = (EquivalencyCountries / len(mf) * 100)
print("\n\nThe % of The Countries \nThat Were Equal Was \n====================\n",
      "{:.2f}%".format(WhereEquivalent))
